...
static int process_environment(char *name, char *value, int append_mode, t_shell *shell);
static int find_export_index(char *name, t_shell *shell);
static char *create_new_value(char *name, char *value, int append_mode, t_shell *shell);
static int update_environment(char *name, char *new_value, int index, t_shell *shell);
								...
								t_shell *shell)
{
	...
	{
		shell->export_size++;
		shell->export_list = realloc(shell->export_list,
				sizeof(char *) * (shell->export_size + 1));
...
//         {
//             shell->export_size++;
//             shell->export_list = realloc(shell->export_list, sizeof(char *) * (shell->export_size + 1));
//             shell->export_list[shell->export_size] = NULL;
//         }
